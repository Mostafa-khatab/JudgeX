...
const updateState = async (req, res) => {
  try {
    ...
    if (interview.status === 'finished') {
      return res.status(403).json({ success: false, message: 'Interview is finished and cannot be modified' });
    }
...
const addMessage = async (req, res) => {
  try {
    ...
    if (interview.status === 'finished') {
      return res.status(403).json({ success: false, message: 'Cannot add messages to a finished interview' });
    }
...
const takeSnapshot = async (req, res) => {
  try {
    ...
    if (interview.status === 'finished') {
      return res.status(403).json({ success: false, message: 'Cannot take snapshots for a finished interview' });
    }
...
const addQuestion = async (req, res) => {
  try {
    ...
    if (interview.status === 'finished') {
      return res.status(403).json({ success: false, message: 'Cannot add questions to a finished interview' });
    }
