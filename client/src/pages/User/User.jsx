					...
					{loading ? (
						...
						<>
							<div className="flex flex-col items-center text-center">
								...
								<h2 className="mt-4 text-2xl font-bold capitalize text-gray-800 dark:text-white">
									{user.fullname || user.name || 'Unknown User'}
								</h2>
								<p className="text-gray-500 dark:text-gray-400">@{user.name || 'unknown'}</p>
							</div>
							<div className="mt-8 space-y-3">
								<div className="flex items-center rounded-lg bg-gray-50 p-3 dark:bg-neutral-700">
									...
									<div className="ml-3">
										...
										<p className="font-semibold text-gray-800 dark:text-white">#{user.top || 'N/A'}</p>
									</div>
								</div>
								<div className="flex items-center rounded-lg bg-gray-50 p-3 dark:bg-neutral-700">
									...
									<div className="ml-3">
										...
										<p className="font-semibold text-gray-800 dark:text-white">{user.totalScore || 0}p</p>
									</div>
								</div>
								<div className="flex items-center rounded-lg bg-gray-50 p-3 dark:bg-neutral-700">
									...
									<div className="ml-3">
										...
										<p className="font-semibold text-gray-800 dark:text-white">{user.totalAC || 0}</p>
									</div>
								</div>
								<div className="flex items-center rounded-lg bg-gray-50 p-3 dark:bg-neutral-700">
									...
									<div className="ml-3">
										...
										<p className="font-semibold text-gray-800 dark:text-white">{user.totalAttempt || 0}</p>
									</div>
								</div>
								<div className="flex items-center rounded-lg bg-gray-50 p-3 dark:bg-neutral-700">
									...
									<div className="ml-3">
										...
										<p className="font-semibold text-gray-800 dark:text-white">
											{user.createdAt ? new Intl.DateTimeFormat('vi-VN', {
												day: '2-digit',
												month: '2-digit',
												year: 'numeric',
						...
						<div className="flex flex-col items-center justify-center py-12 text-center">
							...
							<h3 className="mb-2 text-lg font-semibold text-gray-800 dark:text-white">User Not Found</h3>
					...
					<Tabs defaultValue={searchParams.get('tab') || '1'} className="h-full w-full">
											...
											: problems && Object.keys(problems).length > 0
												? Object.entries(problems).map(([id, item], index) => (
														...
														>
															...
															<div className="flex-1">
																<p className="font-medium capitalize text-gray-800 dark:text-gray-100">{item.name || 'Unknown Problem'}</p>
															</div>
															<span
															...
															>
																{t(item.difficulty || 'unknown')}
															</span>
														</Link>
													))
												: (
													<div className="flex flex-col items-center justify-center py-12 text-center">
														<div className="mb-4 rounded-full bg-gray-100 p-4 dark:bg-neutral-700">
															<img src={ProblemImg} alt="problem" className="size-8 text-gray-400" />
														</div>
														<h3 className="mb-2 text-lg font-semibold text-gray-800 dark:text-white">No Problems</h3>
														<p className="text-gray-500 dark:text-gray-400">This user hasn't attempted any problems yet.</p>
													</div>
												)}
									</div>
								</div>
						...
						<TabsContent value="4" className="mt-0">
							<div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
								{loading ? (
								...
								) : user?.joinedContest && user.joinedContest.length > 0 ? (
									...
									<div className="col-span-full flex flex-col items-center justify-center py-12 text-center">
										<div className="mb-4 rounded-full bg-gray-100 p-4 dark:bg-neutral-700">
											<img src={ContestImg} alt="contest" className="size-8 text-gray-400" />
										</div>
										<h3 className="mb-2 text-lg font-semibold text-gray-800 dark:text-white">No Contests</h3>
										<p className="text-gray-500 dark:text-gray-400">This user hasn't joined any contests yet.</p>
									</div>
								)}
							</div>
						</TabsContent>
